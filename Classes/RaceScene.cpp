...
{
    return RaceScene::create();
}



void RaceScene::initializeOpponents() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 0) {
            opponent->setPosition(Vec2(4090, 180)); 
        }
        if (i == 1) {
            opponent->setPosition(Vec2(3830, -55)); 
        }
        if (i == 2) {
            opponent->setPosition(Vec2(4090, -130)); 
        }
...
void RaceScene::updateOpponents(float delta) {
    float maxOpponentSpeed = 15.0f;
...
Vec2 RaceScene::getNextWaypoint(int opponentIndex) {
    return opponentData[opponentIndex].waypoints[opponentData[opponentIndex].currentWaypoint];
}

float RaceScene::calculateAngleToWaypoint(const Vec2& currentPos, const Vec2& targetPos) {
...
void RaceScene::checkPowerUpCollisions() {
...
void RaceScene::activatePowerUp(const std::string& type) {
    if (type == "speed") {
...
void RaceScene::updatePowerUps(float delta) {
...
void RaceScene::setupMinimap() {
    ...
    minimapTrack = Sprite::create("race1.png"); 
...
void RaceScene::showCarSelectionMenu()
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
...
void RaceScene::initializeRaceScene()
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    ...
    finishLineSprite->setPosition(Vec2(3952, 550));
    finishLineSprite->setScale(5.35f);
...
bool RaceScene::isKeyPressed(EventKeyboard::KeyCode keyCode) {
    return keyMap[keyCode];
}

void RaceScene::update(float delta)
{
    if (!raceStarted) {
        return; 
    }
    if (!hasCollision) {
        if (isKeyPressed(EventKeyboard::KeyCode::KEY_W)|| isKeyPressed(EventKeyboard::KeyCode::KEY_UP_ARROW)) {
            ...
            //AudioEngine::play2d("C:/Users/crisl/OneDrive/Documentos/carreras/Resources/aceler.mp3");
        }
...
void RaceScene::showEndGameMenu(bool playerWon) {
